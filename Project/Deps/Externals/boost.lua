-- https://github.com/xmake-io/xmake/issues/5044
...
package("ame.boost")
    ...
    for _, libname in ipairs(libnames) do
        local enabled = libname == "filesystem" or
                        libname == "system" or
                        libname == "program_options" or
                        libname == "serialization"
        add_configs(libname,    { description = "Enable " .. libname .. " library.", default = enabled, type = "boolean"})
    end
    ...
    on_install("macosx", "linux", "windows", "bsd", "mingw", "cross", function (package)
        ...
        if package:is_plat("windows") then
            host_toolchain = package:toolchain("msvc") or
        ...
        local runenvs
        if package:is_plat("windows") then
            build_toolchain = package:toolchain("msvc") or
                toolchain.load("msvc", {plat = package:plat(), arch = package:arch()})
            if not build_toolchain:check() then
                build_toolchain = package:toolchain("clang-cl") or 
                    toolchain.load("clang-cl", {plat = "windows", arch = os.arch()})
            end
                print("build toolchain: %s", build_toolchain:name())
