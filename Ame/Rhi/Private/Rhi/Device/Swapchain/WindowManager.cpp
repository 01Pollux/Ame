...
namespace Ame::Rhi
{
    ...
    WindowManager::~WindowManager()
    {
        ...
        if (m_SwapChain)
        {
            auto& deviceWrapper = m_ResourceAllocator.get().GetWrapper();
    ...
    void WindowManager::RecreateSwapchain()
    {
        ...
        auto& presentQueue  = deviceWrapper.GetGraphicsQueue();
    ...
    void WindowManager::CreateSwapChainViews()
    {
        ...
        for (uint32_t i = 0; i < count; i++)
        {
            ...
            m_BackBuffers[i].Resource = Texture(m_ResourceAllocator, backBuffers[i]);

            nri::Descriptor* view = nullptr;
            ThrowIfFailed(nriCore.CreateTexture2DView(viewDesc, view), "Failed to create texture view of the swapchain texture");
            m_BackBuffers[i].View = ResourceView(m_ResourceAllocator, view);
        }
    }
