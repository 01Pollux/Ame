...
namespace Ame::Rhi
{
    ...
    [[nodiscard]] void GetOrDefault(Ty& finalValue, const Ty& value) noexcept
    {
    ...
    struct GenericDeviceCreator
    {
            ...
            const create_struct_type& createDescDev)
        {
            ...
            if (createDesc.Surface)
            {
                ...
                auto swapchainDesc  = CreateDiligentSwapChainDesc(*createDesc.Surface->Window, surfaceDesc.Swapchain);
                auto fullscreenDesc = CreateDiligentFullscreenDesc(*createDesc.Surface->Window, surfaceDesc.FullscreenMode);

                traits_type::CreateSwapchain(factoryDev, renderDevice, deviceContext, swapchainDesc, fullscreenDesc, GetDiligentNativeWindow(*surfaceDesc.Window), &swapchain);
                if (!swapchain)
                {
                ...
                windowWrapper = WindowWrapper(surfaceDesc.Window, std::move(swapchain));
            }
